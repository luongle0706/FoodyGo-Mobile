...
class _AddToppingSectionState extends State<AddToppingSection> {
  ...
  SavedUser? _user;
  final AddonSectionRepository addonSectionRepository = AddonSectionRepository.instance;
  ...
  Future<void> saveToppingSection() async {
    ...
    final Map<String, dynamic> body = {
      ...
      "maxChoice":
          isRequired ? selectedMaxOptions : 0, 
      "required": isRequired,
      "addonItems": _toppings, 
    };

    var isCreated = await addonSectionRepository.createAddonSection(_user!.token, body);
    if (isCreated) {
      ScaffoldMessenger.of(context).showSnackBar(
            ...
            Row(
              ...
              children: [
                ElevatedButton(
                  onPressed: () async {
                    bool? result =
                        await context.push<bool>('/protected/add-topping-item');
